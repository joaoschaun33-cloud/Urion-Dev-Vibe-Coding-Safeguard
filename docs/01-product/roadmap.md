...
| 2.3 | Check de `.env`/segredos versionados (R4/R5) | `.env` no git / segredos hardcoded                                                                | Bloqueia commit de segredo        | P       | ✅ (`detect-env-leaks.ts` detecta; CLI `urion-checks --strict` sai com erro — **mas não está ligado ao `.husky/pre-commit`**, então hoje não "bloqueia commit" de fato, só reporta sob demanda)                                                               |
...
| 2.5 | Scripts de guardrail (hooks)                 | Portar `check-secrets`, `check-auth` (e RLS) como hooks pre-commit/PostToolUse                    | Hook bloqueia de verdade          | M       | ❌ Não iniciado — `.husky/pre-commit` roda só `lint-staged` + `test:smoke` + `cursor-doctor`; `urion-checks` não é chamado no hook                                                                                                                            |
...
| #   | Item                         | Entregável                                                                                                         | Pronto quando                   | Esforço | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| --- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------ | ------------------------------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 3.1 | Coverage real                | Substituir proxy `testFiles/codeFiles` por coverage do vitest                                                      | Score usa cobertura medida      | P       | ❌ Não iniciado — **medido em 2026-09-17** via `npm run test:coverage`: **67.98% linhas/statements, 76% branches, 76.04% funções**. Abaixo do próprio limiar do projeto (`vitest.config.ts` exige 85% linhas/statements/funções e 80% branches — falha hoje) e abaixo do dogma de 80% (AGENTS.md, vision.md). `src/shared/domain/scanner-verdict.ts:24-34` ainda usa a proxy `testFiles/codeFiles`, não a cobertura real. **Agravante**: `.github/workflows/ci.yml` só roda `npm run test` e `test:smoke` — nunca `test:coverage` — então essa falha de limiar não trava o CI hoje. |
