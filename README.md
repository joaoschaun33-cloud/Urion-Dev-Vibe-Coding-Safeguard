...
> **Estado (honesto):** projeto jovem, **ainda não validado com usuários reais nem medido contra apps vulneráveis de verdade**. A detecção é por regras/heurísticas (regex) e tem falsos negativos: num corpus interno de 199 casos (feito por nós, portanto otimista), o `vibeguard` encontra **68%** dos casos vulneráveis e o `urion-checks` **62%** — [números e erros por regra](benchmarks/RESULTS.md). Medido em código real (231 repositórios públicos de projetos Lovable, 2026-09-24): antes das correções, ~2 em cada 3 alertas do `urion-checks` eram falso alarme. Depois de corrigir as causas, em projetos **nunca vistos** a precisão esperada é de **~55–70%** (a cada 10 alertas, 3 a 4 não valem a pena); o **recall real é desconhecido** (para credencial em web storage era ~33% antes da última ampliação) — [detalhes e limites](benchmarks/real/RESULTS.md). Este repositório **não** exibe o selo "Grade A" porque o próprio `npm run launch:gate` ainda o reprova.
